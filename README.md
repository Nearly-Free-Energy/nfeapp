...
Step 3 adds server-side customer mapping on top of that verified identity flow. The current mapping lives in [server/customer-map.ts](/Users/atushabe/NearlyFreeEnergy/NFE%20Web%20App/server/customer-map.ts). During this transition phase, an email should still be present in `VITE_ALLOWED_EMAILS`, and it must also be mapped in the customer map before the dashboard is shown.
