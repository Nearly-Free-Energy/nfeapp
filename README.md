...
`npm run test:smoke:deployed` is the deployed-site smoke suite. It runs against a Vercel deployment URL and is intended for preprod verification before promoting to production.
...
- No dedicated preprod domain on Hobby; use the staged deployment URL as the preprod environment

On the Hobby plan, keep the workflow simple:

- merge to `main`
- let Vercel build the deployment
- validate the staged deployment at its generated Vercel URL
...
6. Add only the live production custom domain.
7. Use the deployment URL generated by Vercel as the temporary preprod environment for each release candidate.

### Deployment smoke test

This repository includes a Playwright smoke suite for release-candidate deployments.

Run it locally against a Vercel deployment URL:

```bash
SMOKE_BASE_URL=https://your-deployment-url.vercel.app npm run test:smoke:deployed
```

Or run it from GitHub Actions:

1. Open `Actions` in GitHub.
2. Select `Smoke Deployment`.
3. Click `Run workflow`.
4. Paste the full Vercel deployment URL.
5. Review the run result and Playwright report before promotion.
...
2. Open the staged deployment URL in Vercel and verify:
   ...
   - the Playwright smoke suite passes against the deployment URL
